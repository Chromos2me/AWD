...
help [x]      show command x's usage and description
    ...
    def help_add(self):
        print(
        "add : usage: add [shell path] [pwd] [type(eval/exec)] [method(get/post)]")
        print(
        "      example: add :8001//a.php x eval get / add :8002//fuckyou.php?pass=xxxx a eval get")
    ...
    def do_load(self, argv):
        try:
            ...
            print(
            "Load failed,please run save fist")
    ...
    def help_clear(self):
        ...
        "clear : usage : clear")
        print(
        "        clear all webshell's path and pwd")
    ...
    def help_ip(self):
        ...
        "ip : usage : x.x.a-b.x")
        print(
        "     set ip list , example:ip 192.168.1.1-24 or ip 192.168.2-45.2")
    ...
    def help_getflag(self):
        print(
        "getflag  : usage : getflag curl xxxx ")
        print(
        "           example: getflag curl https:www.baidu.com")
        print(
        "           if you have input this command and it works ,just run getflag!")
    ...
    def do_getflag(self, argv):
        ...
        if argv == '':
            if command == '':
                print(
                "You should input full command at the first time , usage : getflag curl xxxx")
                return
            else:
                get_flag()
        else:
    ...
    def help_submit(self):
        print(
        "submit : usage(the first time) : submit [url] [cookie](please remove ' ') [data](use '?' replace the flag )")
        print(
        "         example: submit http:xxx.xxx.xx/xx/ JSESSIONID=A6F8;route=6cf03 pid=-1&pidName=&flag=?")
        print(
        "         if you have input this command and it works ,just run submit!")
    ...
    def help_upload(self):
        print(
        "upload : usage: upload <file>(default=shell.php)  #run 'save' at first!")
        print(
        "         upload your file with all eval shells , other file please save in 'auxi/' path")
    ...
    def do_upload(self, argv):
        ...
        if argv == '':
            ...
            try:
                upload("auxi/" + argv)
                print(
                "Upload ok,Don't forget to add the new webshell!")
            except:
    ...
    def help_cupload(self):
        print(
        "Usage : command   #run save first!")
        print(
        "        upload linux command undead webshell")
    ...
    def help_removeip(self):
        print(
        "removeip : usage : removeip x.x.x.x")
        print(
        "           remove your own ip from ip list")
